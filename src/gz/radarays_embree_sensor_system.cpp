...
void RadaraysEmbreeSensorSystem::DeclareReconfigurableParams()
{
  ...
  // update_rate feeds 1.0 / update_rate_ (PostUpdate) -- 0 or negative is
  // a division-by-zero / nonsensical publish timing, not just an unusual
  // tuning choice. Bounded away from 0 rather than just >= 0 for the same
  // reason (flagged in self-review).
...
void RadaraysEmbreeSensorSystem::LoadParams(const std::shared_ptr<const sdf::Element> &_sdf)
{
  ...
  if(_sdf->HasElement("update_rate"))
  {
    ...
    {
      std::cerr << "[RadaraysEmbreeSensorSystem] SDF update_rate " << requested
